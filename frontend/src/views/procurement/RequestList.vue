<template>
  <div class="page-container">
    <div class="page-header">
      ...
      <p class="page-desc">管理采购申请单，提交审批流程</p>
    </div>
    <el-card shadow="never" class="table-card">
      ...
      <el-table :data="tableData" v-loading="tableLoading" border stripe>
        ...
        <el-table-column prop="currentStep" label="审批进度" width="120" align="center">
          <template #default="{ row }">
            <span v-if="row.totalSteps > 0">{{ row.currentStep }} / {{ row.totalSteps }}</span>
            <span v-else>-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="260" align="center" fixed="right">
          <template #default="{ row }">
            ...
            <el-button v-if="row.status === 'draft'" text type="primary" size="small" @click="openEdit(row.id)">编辑</el-button>
            <el-button v-if="row.status === 'draft'" text type="success" size="small" @click="handleSubmit(row.id)">提交</el-button>
            <el-button v-if="row.status === 'pending'" text type="warning" size="small" @click="handleWithdraw(row.id)">撤回</el-button>
            <el-button v-if="row.status === 'draft'" text type="danger" size="small" @click="handleDelete(row.id)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
import { getRequestList, submitRequest, withdrawRequest, deleteRequest } from '@/api/procurement'
import type { ProcurementRequest } from '@/api/procurement'

const router = useRouter()

const tableData = ref<ProcurementRequest[]>([])
...
function openEdit(id: number) {
  router.push(`/procurement/requests/${id}/edit`)
}
...
async function handleSubmit(id: number) {
  try {
    await ElMessageBox.confirm('确认提交该采购申请？提交后将进入审批流程。', '确认提交')
...
async function handleWithdraw(id: number) {
  try {
    await ElMessageBox.confirm('确认撤回该采购申请？', '确认撤回')
...
async function handleDelete(id: number) {
  try {
    await ElMessageBox.confirm('确认删除该采购申请？此操作不可恢复。', '确认删除', { type: 'warning' })
